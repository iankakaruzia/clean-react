...
describe('Login Component', () => {
  ...
  test('Should render error if Authentication fails', async () => {
    const { sut, authenticationSpy } = makeSut()
  ...
  test('Should call SaveAccessToken on success', async () => {
    ...
    await simulateValidSubmit(sut)
    expect(updateCurrentAccountMock.account.accessToken).toBe(authenticationSpy.account.accessToken)
    expect(updateCurrentAccountMock.account.name).toBe(authenticationSpy.account.name)
    expect(history.length).toBe(1)
    expect(history.location.pathname).toBe('/')
  })

  test('Should render error if SaveAccessToken fails', async () => {
